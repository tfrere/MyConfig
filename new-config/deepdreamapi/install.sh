...
# CAFFE > https://caffe.berkeleyvision.org/install_apt_debian.html
...
sudo echo "deb http://ftp2.cn.debian.org/debian sid main contrib non-free" >> /etc/apt/sources.list
sudo apt update
sudo apt install caffe-cuda ffmpeg # [ caffe-cpu | caffe-cuda ]
...
# sudo nano /etc/fstab
#
# echo "/home/swapfile.img swap    swap    defaults        0       0" >> /etc/fstab
